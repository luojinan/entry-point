...
import { readJianguoyunText } from "@/lib/server/jianguoyun";
import {
  jianguoyunErrorResponse,
  jianguoyunInvalidInputResponse,
} from "@/lib/server/jianguoyun-api";

export const Route = createFileRoute("/api/jianguoyun/read")({
  server: {
    handlers: {
      ...
      GET: async ({ request, context }) => {
        ...
        try {
          const result = await readJianguoyunText(parsed.data.path, env);
          return jsonResponse(result, 200, {
            "X-Request-Id": result.requestId,
          });
        } catch (error) {
          console.error("Jianguoyun read error:", error);
          return jianguoyunErrorResponse(
            error,
            "Failed to read Jianguoyun file",
          );
        }
      },
    },
