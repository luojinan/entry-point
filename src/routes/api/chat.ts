    ...
    handlers: {
      ...
      POST: async ({ request, context }) => {
        ...
        allTools.finalAnswer = withApproval(
          tool({
            description:
              "当已经完成必要工具调用，并准备向用户输出最终答复时调用。调用后停止继续执行工具循环，前端会把 answer 渲染为最终答复。",
            inputSchema: z.object({
              answer: z
                .string()
                .min(1)
                .describe("展示给用户的最终答复，支持 Markdown。"),
            }),
