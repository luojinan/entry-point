...
class ShandianPlugin extends BasePlugin {
  ...
  private async _fetchDetailLinks(itemID: string): Promise<Link[]> {
    ...
    try {
      ...
      $("#download-list .module-row-one").each((i, s) => {
        ...
        if (
          clipboardText &&
          UC_LINK_REGEX.test(clipboardText) &&
          !seenURLs.has(clipboardText)
        ) {
          seenURLs.add(clipboardText);
          links.push({
            type: "uc" as CloudType,
            url: clipboardText,
            password: "",
          });
        }
  ...
  async search(
    ...
    // 提取搜索结果
    const rawResults: SearchResultWithItemID[] = [];
