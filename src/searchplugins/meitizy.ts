...
class MeitizyPlugin extends BasePlugin {
  ...
  async search(
    ...
    const apiResp: any = await resp.json();
   ...
   */
  _convertToSearchResults(items: any[]): SearchResult[] {
   ...
   */
  _mapLinkType(apiLinkType: string): CloudType {
   ...
   */
  _determineCloudTypeFromURL(url: string): CloudType {
    ...
    if (url.includes("magnet:")) return "others";
    if (url.includes("ed2k://")) return "others";
    return "others";
  }
