...
const LINK_REGEXES: LinkRegex[] = [
  ...
  {
    reg: /https?:\/\/pan\.xunlei\.com\/s\/[0-9a-zA-Z_-]+(\?pwd=[0-9a-zA-Z]+)?/,
    type: "xunlei",
  },
  ...
  { reg: /https?:\/\/share\.weiyun\.com\/[0-9a-zA-Z]+/, type: "others" },
  {
  ...
  { reg: /ed2k:\/\/\|file\|.+\|\d+\|[0-9a-fA-F]{32}\|\//, type: "others" },
];

const MAX_CONCURRENCY = 20;

interface ZhizhenSearchItem extends SearchResult {
  _itemID: string;
}
...
class Zhizhen extends BasePlugin {
  ...
  async search(
    ...
    const searchItems: ZhizhenSearchItem[] = [];
    $(".module-search-item").each((i: number, el: cheerio.Element) => {
  ...
  _extractPassword(url: string): string {
    const matches = url.match(PASSWORD_REGEX);
    return matches ? matches[1] : "";
  }
}

export default Zhizhen;
