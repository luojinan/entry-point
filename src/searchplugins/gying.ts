...
class Gying extends BasePlugin {
   ...
   * @param cookie - Cookie string for gying.net
   */
  ...
  async search(
    ...
    const cookie = (ext.cookie as string) || this._cookie || "";
    ...
    try {
      ...
      throw new Error(`Failed to parse search data: ${(e as Error).message}`);
    }
  ...
  private async _fetchAllDetails(
    ...
    for (let i = 0; i < indices.length; i += MAX_CONCURRENT) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(async (index) => {
          ...
          const detail = await this._fetchDetail(
            resourceID,
            resourceType,
            headers,
          );
  ...
  private async _fetchDetail(
    ...
    try {
      const detailURL = DETAIL_URL_TEMPLATE.replace("%s", resourceType).replace(
        "%s",
        resourceID,
      );
  ...
  private _buildResult(
    ...
    let year = 0;
    if (
      index < (searchData.l.year || []).length &&
      searchData.l.year![index] > 0
    ) {
    ...
    if (
      index < (searchData.l.daoyan || []).length &&
      searchData.l.daoyan![index]
    ) {
      contentParts.push(`导演: ${searchData.l.daoyan![index]}`);
    }
    if (
      index < (searchData.l.zhuyan || []).length &&
      searchData.l.zhuyan![index]
    ) {
    ...
    const datetime = this._parseUpdateTime(
      detail.panlist ? detail.panlist.time : [],
    );
  ...
  private _extractPanLinks(detail: DetailData): Link[] {
    const links: Link[] = [];
  ...
  private _parseRelativeTimeStr(timeStr: string): string {
