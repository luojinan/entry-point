...
class Lou1Plugin extends BasePlugin {
  ...
  async _fetchDetail(detailURL) {
    ...
    const resp = await fetchWithRetry(detailURL, { headers }, { timeout: 12000, retries: 2 });
    const html = await resp.text();
    const $ = cheerio.load(html);
  ...
  _mergeTags(a, b) {
    const set = new Set();
    (a || []).forEach(t => { if (t.trim()) set.add(t.trim()); });
    (b || []).forEach(t => { if (t.trim()) set.add(t.trim()); });
