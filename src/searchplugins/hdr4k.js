...
class Hdr4kPlugin extends BasePlugin {
  ...
  async search(keyword, ext = {}) {
    ...
    $('.slst.mtw ul li.pbw').each((i, el) => {
      ...
      // Check keywords
      const matched = keywords.every(kw => lowerTitle.includes(kw) || lowerContent.includes(kw));
      if (!matched) return;
    ...
    const tasks = items.map(async (item) => {
      try {
        ...
        const { links, detailContent } = await this.getLinksFromDetail(item.postID);
  ...
  isEmptyRequestPost(title, links) {
    const lowerTitle = title.toLowerCase();

    if (links.length > 0) return false;

    const emptyRequestKeywords = ['\u6C42\u7247', '\u6709\u8D44\u6E90\u5417', '\u6709\u6CA1\u6709\u8D44\u6E90', '\u8DEA\u6C42', '\u6C42\u8D44\u6E90'];
    for (const kw of emptyRequestKeywords) {
      if (lowerTitle.includes(kw)) return true;
    }

    const cloudRequestKeywords = ['\u6C42\u963F\u91CC\u4E91\u76D8', '\u6C42\u767E\u5EA6\u7F51\u76D8', '\u6C42\u5938\u514B\u7F51\u76D8', '\u6C42\u8FC5\u96F7\u7F51\u76D8', '\u6C42\u5929\u7FFC\u4E91\u76D8'];
  ...
  async getLinksFromDetail(postID) {
    ...
    try {
      ...
      for (const selector of contentSelectors) {
        $(selector).each((i, el) => {
          if (!detailContent) {
            let content = this.cleanHTMLText($(el).text()).trim();
