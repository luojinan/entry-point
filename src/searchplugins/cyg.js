...
class CygPlugin extends BasePlugin {
  constructor() {
    ...
    this.urlPatterns = [
      ...
      { regex: /https?:\/\/(www\.)?(aliyundrive\.com|alipan\.com)\/s\/[0-9a-zA-Z]+/, type: 'aliyun' },
      { regex: /https?:\/\/pan\.xunlei\.com\/s\/[0-9a-zA-Z_\-]+/, type: 'xunlei' },
      { regex: /https?:\/\/cloud\.189\.cn\/t\/[0-9a-zA-Z]+/, type: 'tianyi' },
      { regex: /https?:\/\/115\.com\/s\/[0-9a-zA-Z]+/, type: '115' },
      { regex: /https?:\/\/(caiyun\.feixin\.10086\.cn|caiyun\.139\.com|yun\.139\.com|cloud\.139\.com|pan\.139\.com)\/.*/, type: 'mobile' },
    ...
    this.nameMap = {
      '夸克': 'quark', '夸克网盘': 'quark',
      'uc': 'uc', 'uc网盘': 'uc',
      '百度网盘': 'baidu', '百度': 'baidu', 'baidu': 'baidu',
      '阿里云盘': 'aliyun', '阿里': 'aliyun', 'aliyun': 'aliyun', '阿里网盘': 'aliyun',
      '迅雷': 'xunlei', '迅雷网盘': 'xunlei', 'xunlei': 'xunlei',
      '天翼': 'tianyi', '天翼云盘': 'tianyi', '189': 'tianyi', '189云盘': 'tianyi',
      '115': '115', '115网盘': '115',
      '移动云盘': 'mobile', '移动': 'mobile', 'mobile': 'mobile', '和彩云': 'mobile', '139云盘': 'mobile', '139': 'mobile', '中国移动云盘': 'mobile',
      '123网盘': '123', '123pan': '123', '123': '123',
      'pikpak': 'pikpak', 'pikpak网盘': 'pikpak',
  ...
  async search(keyword, ext = {}) {
    // Parse options
    const perPage = (ext && ext.per_page) || 20;
    const page = (ext && ext.page) || 1;
    const orderBy = (ext && ext.order_by) || 'date';
    const order = (ext && ext.order) || 'desc';
  ...
  async _getDownloadLinks(postID) {
  ...
  _determineCloudTypeByName(name) {
