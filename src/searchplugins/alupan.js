...
class AlupanPlugin extends BasePlugin {
  constructor() {
    ...
    this.linkPatterns = [
      ...
      { regex: /https?:\/\/www\.aliyundrive\.com\/s\/[0-9A-Za-z]+/, type: 'aliyun' },
      { regex: /https?:\/\/www\.aliyundrive\.com\/drive\/folder\/[0-9A-Za-z]+/, type: 'aliyun' },
    ];
    this.pwdPatterns = [
      /提取码[:：]?\s*([0-9A-Za-z]+)/,
      /密码[:：]?\s*([0-9A-Za-z]+)/,
  ...
  async search(keyword, ext = {}) {
    ...
    $('article.excerpt').each((_, el) => {
      ...
      articleItems.push({ title, detailURL, articleID, summary, tags, publishTime });
    });
