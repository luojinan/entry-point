...
class KkmaoPlugin extends BasePlugin {
  ...
  async search(
    ...
    $("article.excerpt").each((i: number, el: any) => {
      const s = $(el);
    ...
    const results = settled.filter(Boolean) as SearchResult[];

    return filterByKeyword(results, keyword);
  }

  extractArticleID(detailURL: string): string {
    const matches = ARTICLE_ID_REGEX.exec(detailURL);
    if (matches && matches.length >= 2) return matches[1];
    return "";
  }

  parsePublishTime(value: string): string {
  ...
  extractQuarkLinks($: any): Link[] {
    const results: Link[] = [];
    const seen = new Set<string>();

    $(".article-content a[href]").each((i: number, el: any) => {
  ...
  extractPasswordFromLink($: any, link: any): string {
    // Check link text
  ...
  matchPassword(text: string): string {
