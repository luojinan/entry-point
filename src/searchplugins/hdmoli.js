...
class HdmoliPlugin extends BasePlugin {
  ...
  parseResultItem($, s, index) {
    ...
    return {
      uniqueId: generateUniqueID(PLUGIN_NAME, String(index), String(Date.now())),
      title,
      content,
      links: [],
  ...
  extractCategoryInfo($, s) {
    let category = '', region = '', year = '';
  ...
  async fetchDetailLinks(searchResults) {
  ...
  parseNetworkDiskLinks(htmlContent) {
    const links = [];
  ...
  parseNetworkDiskLinksWithRegex(htmlContent) {
    const links = [];
    ...
    while ((match = baiduPattern.exec(htmlContent)) !== null) {
      ...
      links.push({ type: 'baidu', url: match[1], password });
    }

    return links;
  }

  extractPasswordFromBaiduURL(panURL) {
