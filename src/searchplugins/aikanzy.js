const { BasePlugin, filterByKeyword, fetchWithRetry, cleanHTML } = require('./base');
...
class AikanzyPlugin extends BasePlugin {
  constructor() {
    ...
    this.ucLinkRegex = /https?:\/\/drive\.uc\.cn\/s\/[0-9a-zA-Z]+(\?[^"'\s]*)?/g;
  ...
  async search(keyword, ext = {}) {
    ...
    $('article.post-list.contt.blockimg').each((i, el) => {
      ...
      let title = $(el).find('header.entry-header span.entry-title a').text().trim();
      ...
      articleItems.push({ id: articleID, title, detailURL, category, publishDate, viewCount, summary });
    });
    ...
    const detailPromises = articleItems.map(item =>
      this._fetchDetailPageLinks(item.detailURL).catch(() => [])
    );
  ...
  async _fetchDetailPageLinks(detailURL) {
    ...
    $("a[href*='pan.quark.cn'], a[href*='drive.uc.cn'], a[href*='pan.baidu.com'], a[href*='pan.xunlei.com']").each((i, el) => {
    ...
    if (links.length === 0) {
      ...
      const regexes = [
        { regex: this.quarkLinkRegex, type: 'quark' },
        { regex: this.ucLinkRegex, type: 'uc' },
        { regex: this.baiduLinkRegex, type: 'baidu' },
        { regex: this.xunleiLinkRegex, type: 'xunlei' },
      ];
  ...
  _extractPasswordFromURL(urlStr) {
