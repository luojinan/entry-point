...
class CldiPlugin extends BasePlugin {
  constructor() {
    ...
    this.maxPages = 5;
    this.adRegex = /【[^】]*】/g;
    this.fileSizeRegex = /^(.+?)&nbsp;<span class="lightColor">([^<]+)<\/span>$/;
    this.categoryMap = {
      '影视': '影视',
      '音乐': '音乐',
      '图像': '图像',
  ...
  async search(keyword, ext = {}) {
    ...
    try {
      ...
      throw new Error(`[${this.name}] search page 1 failed: ${e.message}`);
    }
    ...
    if (this.maxPages > 1) {
      ...
      for (let page = 2; page <= this.maxPages; page++) {
        pagePromises.push(this._searchPage(keyword, page).catch(() => []));
      }
  ...
  async _searchPage(keyword, page) {
  ...
  _parseSearchResult($, el) {
    ...
    if (magnetLink) {
      result.links = [{ type: 'magnet', url: magnetLink, password: '' }];
    }
