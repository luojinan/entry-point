...
const QUARK_LINK_REGEX =
  /https?:\/\/pan\.quark\.cn\/s\/[0-9a-fA-F]+(\?pwd=[0-9a-zA-Z]+)?/;
const MAGNET_LINK_REGEX = /magnet:\?xt=urn:btih:[0-9a-fA-F]{40}[^"'\s]*/g;
const ED2K_LINK_REGEX = /ed2k:\/\/\|file\|[^|]+\|[^|]+\|[^|]+\|\/?/g;

const PAN_LINK_REGEXES: Record<string, RegExp> = {
  baidu:
    /https?:\/\/pan\.baidu\.com\/s\/[0-9a-zA-Z_-]+(?:\?pwd=[0-9a-zA-Z]+)?(?:&v=\d+)?/g,
...
class Fox4k extends BasePlugin {
  ...
  async search(
    ...
    const { results: firstPageResults, totalPages } = await this._searchPage(
      encodedKeyword,
      1,
    );
    let allResults: SearchResultWithId[] = [...firstPageResults];
    ...
    if (maxPagesToSearch > 1) {
      ...
      for (let page = 2; page <= maxPagesToSearch; page++) {
        pagePromises.push(
          this._searchPage(encodedKeyword, page).catch(() => ({
            results: [],
            totalPages: 0,
          })),
        );
  ...
  private async _searchPage(
    ...
    page: number,
  ): Promise<{ results: SearchResultWithId[]; totalPages: number }> {
    const searchURL =
      page === 1
        ? SEARCH_URL.replace("%s", encodedKeyword)
        : SEARCH_PAGE_URL.replace("%s", encodedKeyword).replace(
            "%d",
            page.toString(),
          );
  ...
  private async _getDetailInfo(id: string): Promise<DetailInfo | null> {
    try {
      ...
      const detail: DetailInfo = { links: [], tags: [], content: "" };
      ...
      // Get plot
      detail.content = (
        $(".hl-content-wrap .hl-content-text").text() || ""
      ).trim();
