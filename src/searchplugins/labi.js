...
class LabiPlugin extends BasePlugin {
  ...
  async _fetchDetailLinks(itemID) {
    ...
    $('#download-list .module-row-one').each((i, el) => {
      ...
      if (clipboardText && this._isValidURL(clipboardText) && quarkLinkRegex.test(clipboardText)) {
   ...
   */
  _isValidURL(url) {
    if (!url || url.includes('javascript:') || url.includes('#') || !url.startsWith('http')) {
      return false;
    }
