...
const { BasePlugin, generateUniqueID, determineCloudType, fetchWithRetry, filterByKeyword } = require('./base');
...
class Ddys extends BasePlugin {
  ...
  _parseNetworkDiskLinks(htmlContent) {
    ...
    for (const pat of patterns) {
      let match;
      while ((match = pat.pattern.exec(htmlContent)) !== null) {
        const url = match[1];
  ...
  _extractPassword(content, panURL) {
    const patterns = [
