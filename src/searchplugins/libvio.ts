...
class LibvioPlugin extends BasePlugin {
  ...
  async search(
    ...
    $("ul.stui-vodlist li").each((i: number, el: any) => {
      const s = $(el);
    ...
    // Filter by keyword
    const filtered: Array<SearchResult & { _detailURL?: string }> =
      filterByKeyword(results, keyword);
  ...
  async _enrichWithDetailLinks(
    ...
    const tasks = results.map(async (r, idx) => {
      try {
        ...
        const links = await this._fetchDetailPageLinks(
          r._detailURL!,
          headers,
          keyword,
        );
  ...
  async _fetchDetailPageLinks(
    ...
    const playLinks: Array<{ url: string; panType: string }> = [];

    $(".stui-vodlist__head").each((i: number, el: any) => {
      const s = $(el);
    ...
    for (const playLink of playLinks) {
      try {
        const panLink = await this._fetchPanLink(
          playLink.url,
          detailURL,
          headers,
        );
   ...
   */
  _mapPanType(from: string, url: string): CloudType {
