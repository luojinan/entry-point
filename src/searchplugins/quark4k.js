...
const { BasePlugin, getRandomUA, cleanHTML, filterByKeyword, deduplicateResults, fetchWithRetry } = require('./base');
...
class Quark4KPlugin extends BasePlugin {
   ...
   * 从文本提取夸克网盘链接
   */
  _extractQuarkLinksFromText(content) {
    ...
    for (const info of linkInfos) {
      ...
      for (const pwInfo of passwordInfos) {
        if (info.category === 'quark' && (pwInfo.keyword === '提取码' || pwInfo.keyword === '密码')) {
