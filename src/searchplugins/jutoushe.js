...
class JutoushePlugin extends BasePlugin {
  ...
  async getDetailLinks(detailURL) {
    try {
      ...
      $('.dlipp-cont-bd a.dlipp-dl-btn').each((i, el) => {
  ...
  extractTags(title) {
    ...
    let match;
    while ((match = categoryPattern.exec(title)) !== null) {
