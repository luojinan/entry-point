...
class Haisou extends BasePlugin {
  ...
  async search(keyword, ext = {}) {
    ...
    if (ext.pages_per_type && typeof ext.pages_per_type === 'number' && ext.pages_per_type > 0) {
  ...
  async _fetchSearchPage(keyword, pageNo, panType) {
  ...
  async _fetchShareLink(hsid, platform) {
  ...
  _buildShareURL(platform, shareCode) {
  ...
  _mapPlatformType(platform) {
    switch ((platform || '').toLowerCase()) {
      case 'ali': return 'aliyun';
