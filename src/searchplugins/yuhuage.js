...
class Yuhuage extends BasePlugin {
  ...
  async search(keyword, ext = {}) {
    ...
    $('.search-item.detail-width').each((i, el) => {
      const s = $(el);
  ...
  async _fetchDetailsParallel(detailURLs, results) {
    ...
    for (let i = 0; i < detailURLs.length; i += MAX_CONCURRENCY) {
      ...
      const batchPromises = batch.map((url, j) => {
        ...
        return this._fetchDetailLinks(url).then(links => {
