...
const { BasePlugin, fetchWithRetry, getRandomUA, generateUniqueID, cleanHTML, extractPassword } = require('./base');
...
const STORAGE_DIR = path.join(process.env.CACHE_PATH || './cache', 'qqpd_users');
...
class QqpdPlugin extends BasePlugin {
   ...
   */
  buildChannelTasks(users) {
    // 1. Collect all channels and their owning users
    const channelOwners = new Map(); // channelID -> [user, ...]
    ...
    const tasks = [];
    const userTaskCount = new Map(); // userHash -> count
    ...
    for (const [channelID, owners] of channelOwners) {
      ...
      let guildID = '';
      if (selectedUser.channel_guild_ids && selectedUser.channel_guild_ids[channelID]) {
   ...
   */
  async resolveGuildID(channelNumber, cookieStr) {
  ...
  async refreshCookie(cookieStr) {
    ...
    try {
      const resp = await fetchWithRetry('https://pd.qq.com/explore', {
        headers: {
          'Cookie': cookieStr,
   ...
   */
  async searchSingleChannel(keyword, cookieStr, channelID, guildID) {
    if (!guildID) return [];
   ...
   */
  extractResultInfo(item, channelID, index) {
  ...
  async search(keyword, ext = {}) {
    ...
    for (let i = 0; i < tasks.length; i += MAX_CONCURRENT_CHANNELS) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(task =>
          this.searchSingleChannel(keyword, task.cookie, task.channelID, task.guildID)
        )
      );
