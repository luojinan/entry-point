...
class DaishudjPlugin extends BasePlugin {
  constructor() {
    ...
    this.linkPatterns = [
      { regex: /https?:\/\/pan\.quark\.cn\/(s|g)\/[0-9A-Za-z]+/, type: 'quark' },
      { regex: /https?:\/\/(?:www\.)?(aliyundrive\.com|alipan\.com)\/s\/[0-9A-Za-z]+/, type: 'aliyun' },
      { regex: /https?:\/\/pan\.baidu\.com\/s\/[0-9A-Za-z\-_]+/, type: 'baidu' },
      { regex: /https?:\/\/pan\.xunlei\.com\/s\/[0-9A-Za-z\-_]+/, type: 'xunlei' },
      { regex: /https?:\/\/drive\.uc\.cn\/s\/[0-9A-Za-z]+/, type: 'uc' },
      { regex: /https?:\/\/(?:www\.)?mypikpak\.com\/s\/[0-9A-Za-z]+/, type: 'pikpak' },
      { regex: /https?:\/\/caiyun\.139\.com\/[^\s]+/, type: 'mobile' },
      { regex: /magnet:\?xt=urn:btih:[0-9A-Za-z]+/, type: 'magnet' },
      { regex: /https?:\/\/(?:www\.)?(123pan\.com|123pan\.cn|123684\.com|123685\.com|123912\.com|123592\.com)\/s\/[0-9A-Za-z]+/, type: '123' },
    ];
  ...
  async search(keyword, ext = {}) {
    ...
    $('.item-jx.item-blog').each((_, el) => {
      ...
      articleItems.push({ title, detailURL, postID, summary, tags, publishTime });
    });
  ...
  _extractPostID(detailURL) {
    const matches = this.idRegex.exec(detailURL);
    return matches ? matches[1] : '';
  }

  _parseChineseDate(value) {
    value = (value || '').trim();
    if (!value) return '';
  ...
  _extractLinks($, selection) {
    ...
    selection.find('a[href]').each((_, node) => {
      ...
      results.push({ type: linkType, url: normalized, password });
    });
    ...
    let match;
    while ((match = this.textURLReg.exec(text)) !== null) {
      const raw = match[0];
      const { type: linkType, normalized } = this._classifyLink(raw);
      if (!linkType || seen.has(normalized)) continue;
      seen.add(normalized);
