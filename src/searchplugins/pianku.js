...
const panLinkRegexes = {
  ...
  '123':  /https?:\/\/(?:www\.)?(?:123pan\.com|123684\.com)\/s\/[0-9a-zA-Z_-]+(?:\?[^"\s]*)?/,
...
class PiankuPlugin extends BasePlugin {
  ...
  async search(keyword, ext = {}) {
    ...
    const finalResults = settled.filter(Boolean);
  ...
  extractSingleResult($, s) {
    ...
    return {
      ...
      title,
      content,
      links: [{ type: 'others', url: fullLink, password: '' }],
      channel: '',
