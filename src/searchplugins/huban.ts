...
interface SearchResultWithItemId extends SearchResult {
  _itemID: string;
}
...
class HubanPlugin extends BasePlugin {
  ...
  async search(
    ...
    const results: SearchResultWithItemId[] = [];
  ...
  private parseSearchItem(
    ...
    // Extract director
    let director = "";
    s.find(".video-info-items").each((i: number, el: any) => {
    ...
    // Extract quality
    const quality = s
      .find(".video-info-header .video-info-remarks")
      .text()
      .trim();
  ...
  private async enhanceWithDetails(
    results: SearchResultWithItemId[],
  ): Promise<SearchResult[]> {
  ...
  private async fetchDetailLinks(itemID: string): Promise<Link[]> {
    ...
    try {
      ...
      $("#download-list .module-row-one").each((i, el) => {
        ...
        if (linkURL && this.isValidNetworkDriveURL(linkURL)) {
          ...
          if (linkType) {
            links.push({ type: linkType as any, url: linkURL, password: "" });
          }
        }
      });
