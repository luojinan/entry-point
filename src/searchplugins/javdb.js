...
class JavdbPlugin extends BasePlugin {
  ...
  parseResultItem($, s, index) {
    ...
    if (releaseDate) contentParts.push(`\u767C\u5E03\u65E5\u671F\uFF1A${releaseDate}`);
    if (tags.length > 0) contentParts.push(`\u6A19\u7C64\uFF1A${tags.join(' ')}`);
  ...
  async fetchDetailMagnetLinks(searchResults) {
    const allResults = [];
      ...
      try {
        ...
        if (magnetLinks.length > 0) {
          ...
          return magnetLinks.map(link => {
            const linkHash = crypto.createHash('md5').update(link.url).digest('hex').substring(0, 8);
  ...
  async fetchDetailPageMagnetLinks(detailURL) {
    try {
