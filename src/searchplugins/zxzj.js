/**
 * zxzj - 在线之家插件
 * 影视资源搜索，从播放页提取网盘链接 (通过player_aaaa变量)
 */
...
class Zxzj extends BasePlugin {
  ...
  async _fetchSearchResults(searchURL) {
  ...
  async _processDetailPages(items) {
    const results = [];
  ...
  async _fetchPanLinks(playLinks) {
    const links = [];
  ...
  async _fetchSinglePanLink(pl) {
    try {
      const resp = await fetchWithRetry(pl.url, {
        method: 'GET',
        headers: this._headers(BASE_URL),
      }, { timeout: 30000, retries: 2 });
  ...
  _determinePanType(panURL, lineType) {
  ...
  _parseUpdateTime(text) {
    const match = text.match(/更新[：:]\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}|\d{4}-\d{2}-\d{2})/);
