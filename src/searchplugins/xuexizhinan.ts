...
class XuexizhinanPlugin extends BasePlugin {
  ...
  async search(
    ...
    $(".url-card").each((i: number, el: cheerio.Element) => {
      const s = $(el);
  ...
  async processDetailPage(detailURL: string): Promise<SearchResult | null> {
    ...
    const quarkLinks: Link[] = [];

    $("li, .site-go a").each((i: number, el: cheerio.Element) => {
      const s = $(el);
    ...
    for (const magnetLink of magnetLinks) {
      links.push({ type: "others", url: magnetLink, password: "" });
    }
...
export default XuexizhinanPlugin;
