...
class Erxiao extends BasePlugin {
  ...
  _parseSearchItem($, el) {
    ...
    // Extract category
    const category = ($el.find('.video-info-items').first().find('.video-info-item').first().text() || '').trim();
    ...
    // Extract year
    const year = ($el.find('.video-info-items').last().find('.video-info-item').first().text() || '').trim();

    // Extract quality
    const quality = ($el.find('.video-info-header .video-info-remarks').text() || '').trim();
