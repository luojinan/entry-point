const { BasePlugin, generateUniqueID, determineCloudType, extractPassword, fetchWithRetry, filterByKeyword } = require('./base');
...
class AhhhhfsPlugin extends BasePlugin {
  ...
  async search(keyword, ext = {}) {
    ...
    $('article.post-item.item-list').each((i, el) => {
      ...
      const tags = [];
      $(el).find('.entry-cat-dot a').each((j, tag) => {
      ...
      articleItems.push({ title, detailURL, articleID, content, tags, datetime });
    });
